<template>
  <div id="index">
    ...
    <nav>
      ...
      <div class="section_title">
        ...
        >
          <div id="addproject_area" v-on:click.stop>
            <h2>プロジェクトを追加する</h2>
            <span @click="closeChannelModal" class="x_button">×</span>
            <p>プロジェクトを追加して新しい開発を進めましょう！</p>
            <input placeholder="リモートリポジトリURL" type="text" v-model="channel">
      ...
      <div :key="user.user_id" class="navigation_content" v-for="user in users">
        <span :class="[isOnline(user) ? 'online_status' : 'offline_status']"></span>
        <span @click="directMessage(user)" class="">{{ user.name }}</span>
      </div>
    </nav>
      ...
      <header>
        ...
        <div id="channel_button" v-if="targetRepository">
          <div v-if="directoryExistence">
            <button @click="gitPull" style="background: #ff4f4c" v-if="localRipository.older">アップデート</button>
            <button style="background: #919191;cursor: default;" v-else>アップデート</button>
            <button @click="openExplorer" v-if="directoryExistence">フォルダを開く</button>
          </div>
          <button @click="gitClone" v-else>ダウンロード</button>
        </div>
      </header>

      <main @dragenter="dragEnter"
            @dragleave="dragLeave"
            @dragover.prevent
      ...
      >
        ...
        >
          <div @click.stop class="z-20 bg-white text-gray-900 w-1/3 rounded-md">
            <div class="w-full h-2">
              <div class="bg-green-900 h-full block" ref="progress_bar" style="width:0%"></div>
            </div>
        ...
        <div class="h-full flex flex-col ml-6">
            ...
            <div id="message_wrapper">
              ...
              >
                ...
                <div>
                  ...
                  <div class="message_category message" v-if="message.category === 'message'">#message</div>
                  <div @click="openCommit(message.commitID)"
                       class="message_category upload"
                       v-if="message.category === 'upload'">
                    #upload
                  </div>
                  <div v-if="message.url">
                    <img height="64px" src="https://image.flaticon.com/icons/png/512/129/129492.png" width="64px"/>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div id="send_message">
            ...
            ></textarea>
            <div id="button_area">
              <button @click="sendMessage" style="background: $main-color">送信
              </button
              >
              <span v-if="directoryExistence">
                                <button @click="gitPush" style="background: #ff4f4c" v-if="uploadAble">アップロード</button>
                                <button style="background: #919191" v-else>アップロード</button>
                            </span>
              <span class="filter_area">
                                <input placeholder="絞り込み検索" type="text" v-model="messagefilter.target"
                                ><button @click="messagefilter.target = null" class="filter_reset"
                                         v-if="messagefilter.target">x</button>
                            </span>
            </div>
          </div>
        </div>
  ...
  export default {
      ...
      gitPush () {
        ...
        this.exe(commands[0], this.localRipository.workDir).on('close', (code) => {
          ...
          this.exe(commands[1], this.localRipository.workDir).on('close', (code) => {
            ...
            const pushChild = this.exe(commands[2], this.localRipository.workDir)
      ...
      directMessage (user) {
        ...
        firebase
          ...
          .on('child_added', snapshot => {
            ...
            this.$nextTick(() => {
              let messageBottom = document.getElementById('message_bottom')
    ...
    mounted () {
      ...
      firebase
        ...
        .ref('connections')
        .on('child_added', snapshot => {
          let newConnection = snapshot.val()
          this.connections.push(newConnection)

          let user = this.users.find(
            user => user.user_id === newConnection.user_id
          )
      ...
      firebase
        ...
        .on('child_removed', snapshot => {
          let removeConnection = snapshot.val()

          this.connections = this.connections.filter(
            connection => connection.key !== removeConnection.key
          )

          let index = this.connections.findIndex(connection => {
            return connection.user_id === removeConnection.user_id
          })

          if (index === -1) {
            let user = this.users.find(
              user => user.user_id === removeConnection.user_id
            )
            user.status = 'offline'
          }
        })
